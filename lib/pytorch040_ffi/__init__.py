#---------------------------------
#This is clone from ffi module from pytorch 0.4.0, which was pytorch wrapper for cffi.
#In pytorch 1.0.1 ffi was deprecated with recomendation to use cpp_extensions instead.
#Here is trial to work around "progressive" intentions.
#---------------------------------
...
try:
    ...
    raise ImportError("pytorch040_ffi requires the cffi package")


if cffi.__version_info__ < (1, 4, 0):
    raise ImportError("pytorch040_ffi requires cffi version >= 1.4, but "
...
def create_extension(name, headers, sources, verbose=True, with_cuda=False,
    ...
    if os.sys.platform == 'win32':
        ...
        import torch
        torch_dir = os.path.abspath(os.path.dirname(torch.__file__))
